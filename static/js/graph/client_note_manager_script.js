...
function addNote(text, parent_id, comment_id, date, isGPT, withUnder) {
        ...
        for (let header of all_headers) {
            ...
            if (header.getElementsByClassName("note-header")[0].dataset.commentId == parent_id) {
                ...
                if (header.getElementsByClassName("note-with-under").length < 1) {
                    header.getElementsByClassName("note-header")[0].classList.add("note-with-under")
                }
                if (header.getElementsByClassName("note-header")[0].dataset.isActivate == "false") {
    ...
    <div style="text-align: right;">
        <button class="handle_button" onclick="gpt_think(${comment_id}, this.parentElement.parentElement)">
        ...
        <button class="handle_button" onclick="openEditModal(this.parentElement.parentElement)">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="#888888" viewBox="0 0 24 24">
        ...
        <button class="handle_button" onclick="openModal(this.parentElement.parentElement.parentElement)">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="#888888" viewBox="0 0 24 24">
            <path d="M19 11H13V5h-2v6H5v2h6v6h2v-6h6z"/>
        </svg>
        </button>

        <button class="handle_button" onclick="open_delete_model(${comment_id})">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="#888888" viewBox="0 0 24 24">
